...
class Simulate:
    ...
    def calculate_pipelined_cycles(self) -> int:
        """
        流水线仿真入口：先根据 build_tiles 拆分 tile，
        再交给 PipelineScheduler 逐周期模拟六阶段并行。
        """
        # 1) 分块
        self.build_tiles()
...
class PipelineScheduler:
    ...
    def _get_stage_latency(self, coord, stage_id):
        ...
        if stage_id == 0: # OUT2
            base = load_A * tile.out2_A + load_B * tile.out2_B
        elif stage_id == 1: # OUT1
            base = tile.out1_C
        elif stage_id == 2: # FIX
            base = tile.fix_C
        elif stage_id == 3: # MTE2
            base = load_A * tile.mte2_A + load_B * tile.mte2_B
        elif stage_id == 4: # MTE1
